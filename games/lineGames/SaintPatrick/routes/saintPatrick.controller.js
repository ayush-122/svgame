...
async function postGamePlaySaintPatrick(req, res) {
  try {
    ...
    const newState = await saintPatrick.stateHandler(nextState, obj.payload);
...
async function postGameContinueSaintPatrick(req, res) {
  try {
    if (req.body.state == "FREE") {
      ...
      const newState = await saintPatrick.stateHandler(nextState, obj.payload, obj.lastState.raw_response);
      //Calculating Win
...
// ? **************** DONE REQUEST ******************
async function postGameDoneSaintPatrick(req, res) {
  await gamePlay.postDone(req, res);
}

// ? **************** RESUME REQUEST ******************
async function postGameResumeSaintPatrick(req, res) {
  await gamePlay.postResume(req, res);
}
module.exports = {
  postGamePlaySaintPatrick,
  postGameContinueSaintPatrick,
  postGameDoneSaintPatrick,
  postGameResumeSaintPatrick,
};
