...
const { changeSymbolCode, expandingWildEnd } = require("../../utils/SlotConstFunct");
...
class FreeGameState extends GameState {
  ...
  async init(jsonRequestData, previousState) {
    ...
    let countWild = new Scatter(symbolMatrix, GameConstant.WILD_SYMBOL).scatterCount(); // ! Counting Wild
    let featureExpandingData = expandingWildEnd(symbolMatrix, countWild.count);
    ...
    responseMap.set(
      ...
      freeTotalWon + previousState.freeSpinTotalWon + previousState.totalCreditsWon
    );
