...
//  **************** BASE GAME PLAY ******************
async function postGamePlayBookOfEgypt(req, res) {
  try {
...
async function postGameContinueBookOfEgypt(req, res) {
  try {
    ...
    // *************** Bonus ****************
      ...
      //Calculating Win
      const credit_amount = newState.totalBonusWin;
      const dbReport = await gamePlay.updateDatabase(newState, req, credit_amount, obj.lastState);
      return res.status(200).json(dbReport);
...
// ? **************** DONE REQUEST ******************
async function postGameDoneBookOfEgypt(req, res) {
  await gamePlay.postDone(req, res);
}

// ? **************** RESUME REQUEST ******************
async function postGameResumeBookOfEgypt(req, res) {
  await gamePlay.postResume(req, res);
}

module.exports = {
  postGamePlayBookOfEgypt,
  postGameContinueBookOfEgypt,
  postGameDoneBookOfEgypt,
  postGameResumeBookOfEgypt,
};
