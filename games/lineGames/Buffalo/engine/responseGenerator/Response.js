...
class ResponseGenerator {
  ...
  createResponse(responseMap, response) {
    ...
    if (bonusTriggerData) {
      ...
      response.bonus.bonusSymbolCount = responseMap.get(BONUS_COUNT);
      response.bonus.bonusSymbolPos = responseMap.get(BONUS_POSITION);
    }
    ...
    if (scatterDataPresent) {
      ...
      if (isRewardFreeGame) {
        response.scatter.freegametrigger = responseMap.get(
          SCATTER_FREE_GAME_TRIGGER
        );
        response.scatter.freeSpinWon = responseMap.get(SCATTER_FREE_SPIN_WON);
        response.freeGame.totalFreeSpin = responseMap.get(
          FREE_GAME_TOTAL_FREE_SPIN
        );
        response.freeGame.currentFreeSpin = responseMap.get(
          FREE_GAME_CURRENT_FREE_SPIN
        );
