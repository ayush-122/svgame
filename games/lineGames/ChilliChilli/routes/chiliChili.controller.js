...
async function postGamePlayChiliChili(req, res) {
  try {
    ...
    const newState = await chiliChili.stateHandler(nextState, obj.payload);
...
async function postGameContinueChiliChili(req, res) {
  try {
    if (req.body.state == "FREE") {
      ...
      const newState = await chiliChili.stateHandler(nextState, obj.payload, obj.lastState.raw_response);
      //Calculating Win
...
// ? **************** DONE REQUEST ******************
async function postGameDoneChiliChili(req, res) {
  await gamePlay.postDone(req, res);
}

// ? **************** RESUME REQUEST ******************
async function postGameResumeChiliChili(req, res) {
  await gamePlay.postResume(req, res);
}
module.exports = {
  postGamePlayChiliChili,
  postGameContinueChiliChili,
  postGameDoneChiliChili,
  postGameResumeChiliChili,
};
