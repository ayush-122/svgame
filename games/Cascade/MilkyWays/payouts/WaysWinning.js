...
function waysWinCalculation(symbolGrid) {
  ...
  for (let i = 0; i < uniqueReel.length; i++) {
    ...
    eachElementCount.push(
      reelSymbols1.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
    eachElementCount.push(
      reelSymbols2.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
    eachElementCount.push(
      reelSymbols3.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
    eachElementCount.push(
      reelSymbols4.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
    eachElementCount.push(
      reelSymbols5.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
    eachElementCount.push(
      reelSymbols6.filter((x) => x === firstElement || x === WILD_SYMBOL).length
    );
