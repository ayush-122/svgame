...
async function processBaseGamePlay(req) {
  ...
  let payload = {
    ...
    nextStateDetails: last_state ? last_state.next_state_details : null,
...
async function addBonusBalance(req, last_state, credit_amount) {
  try {
    ...
    const responseObj = {
      ...
      nextState: next_state == "COMPLETED" ? "BASE" : next_state,
...
async function postDone(req, res) {
  try {
    ...
    return res.status(200).json({ status: true, message: "Spin Status Updated Successfully", data: responseObj });
  } catch (error) {
    console.error(error);
