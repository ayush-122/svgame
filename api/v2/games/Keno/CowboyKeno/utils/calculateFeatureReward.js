const { S1, S2, S3, S4, S5, B1, B2, G1, G2, H1, H2, H3, H4, CN, WD } = require("./../configuration/SymbolConfig");

function countFeatureSymbols(playerNumbers, remainingKenoPool, featureSymbols) {
  const symbolNumbers = remainingKenoPool.filter((element) => !playerNumbers.includes(element));
  const featureStops = [];
