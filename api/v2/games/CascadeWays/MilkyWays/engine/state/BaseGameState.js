  ...
  async init(request) {
    ...
    const { createReelRandomMap, createReelMap } =
      pools.base_reel_weight < 50
        ...
        : require("../../configuration/MainGameReelConfig1"); //need to change the values
    ...
    const starSymbolMatrix = starSymbolGrid.starMatrixCreation(starReelStops); //currently hardcoded for triggering free game (i.e three ) 0's inserted)
    ...
    if (positionOfStars.elementCount >= 3) {
      ...
      const rewardFreeGame = StateUtils.freeGameCalculation(positionOfStars.elementCount);
