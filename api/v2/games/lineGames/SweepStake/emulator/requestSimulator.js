  ...
  try {
    ...
    for (let i = 1; i <= cycles; i++) {
      ...
      while (nextState == "PICK_BONUS") {
        ...
        let wheelStateRes = await stateHandler("PICK_BONUS", req, previousState);
        previousState = wheelStateRes;
        nextState = wheelStateRes.state.next;
        let wheelSpinCreditsWon = wheelStateRes.pickBonus.totalBonusWin;
        // if (!wheelSpinCreditsWon) console.log(wheelStateRes);
        totalWheelWon = totalWheelWon + wheelSpinCreditsWon;
        totalGameWin = totalGameWin + wheelSpinCreditsWon;
      }
      ...
      if (i % 1000000 == 0) {
        ...
        const wheelTotalOdds = cycles / wheelGameOddsAll;

        console.log(
          i,
          `BaseWinPct: ${baseWinPercentage.toFixed(2)}% | FreeWinPct: ${freeWinPercentage.toFixed(
            2
          )}% | WheelWinPct: ${wheelWinPercentage.toFixed(2)}% |  TotalWinPct: ${totalWinPercentage.toFixed(
            2
            ...
            2
          )} | WheelOddsTotal: ${wheelTotalOdds.toFixed(2)}`
        );
      }
