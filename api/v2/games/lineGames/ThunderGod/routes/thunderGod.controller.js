...
async function postGamePlayThunderGod(req, res) {
  try {
    ...
    const newState = await thunderGod.stateHandler(nextState, obj.payload);
...
async function postGameContinueThunderGod(req, res) {
  try {
    if (req.body.state == "FREE") {
      ...
      const newState = await thunderGod.stateHandler(nextState, obj.payload, obj.lastState.raw_response);
      //Calculating Win
...
// ? **************** DONE REQUEST ******************
async function postGameDoneThunderGod(req, res) {
  await gamePlay.postDone(req, res);
}

// ? **************** RESUME REQUEST ******************
async function postGameResumeThunderGod(req, res) {
  await gamePlay.postResume(req, res);
}
module.exports = {
  postGamePlayThunderGod,
  postGameContinueThunderGod,
  postGameDoneThunderGod,
  postGameResumeThunderGod,
};
