  ...
  try {
    ...
    for (let i = 1; i <= cycles; i++) {
      ...
      while (nextState == "FREE" && (firstFreeSpin || currentFreeSpin != totalFreeSpin)) {
        ...
        totalFreeFeatureWon = totalFreeFeatureWon + freeStateRes.featureWin;
        totalFreeWon = totalFreeWon + freeSpinCreditsWon;
        totalGameWin = totalGameWin + freeStateRes.freeGame.freeSpinCreditsWon;
      }

      let firstRespin = true;
      let respinStateRes = {};
      let currentRespin = 0;
      let totalRespin = 0;
      while (nextState == "RESPIN" && (firstRespin || currentRespin != totalRespin)) {
        firstRespin = false;
        // respinGameOddsAll++;
        req = helper.createRespinRequest();
        respinStateRes = await stateHandler("RESPIN", req, previousState);
        currentRespin = respinStateRes.reSpinGame.currentRespin;
        totalRespin = respinStateRes.reSpinGame.totalRespin;
      ...
      if (i % 1000000 == 0) {
        ...
        const freeFeatureWinPercentage = (totalFreeFeatureWon / totalBet) * 100;
        const respinWinPercentage = (totalRespinWon / totalBet) * 100;
        const respinFeatureWinPercentage = (totalRespinFeatureWon / totalBet) * 100;
        ...
        console.log(
          ...
          `BaseWinPct: ${baseWinPercentage.toFixed(2)}% | FreeWinPct: ${freeWinPercentage.toFixed(
            2
          )}% | FreeFeatureWinPct: ${freeFeatureWinPercentage.toFixed(
            2
          )}% | RespinWinPct: ${respinWinPercentage.toFixed(
            2
          )}% | RespinFeatureWinPct: ${respinFeatureWinPercentage.toFixed(
            2
