...
const { changeSymbolCode, expandingWildBox } = require("../../utils/SlotConstFunct");
...
class RespinGameState extends GameState {
  ...
  async init(jsonRequestData, previousState) {
    ...
    let featureExpandingData = expandingWildBox(symbolMatrix, random);
    ...
    if (featureExpandingData.status) {
      ...
      responseMap.set(ResponseConstants.WILD_BOX, featureExpandingData.wildBox);
