...
class BuyFeatureBaseState extends GameState {
  ...
  init(jsonRequestData, previousState) {
    ...
    const reelSymbols = mainGameReelConfig.reel(RTP_LEVEL, reelNumber);

    let symbolGrid = new SymbolGridCreation(DISPLAY_HEIGHT, NUM_REELS, reelSymbols);
    ...
    let FeatureExpandingReels = this.symbolGridExpandingReels(matrix, featureExpandingData.expandingReels);
    ...
    this.responseObj.freeSpinTotalWon = freeTotalWon + previousState.freeSpinTotalWon;
    this.responseObj.totalCreditsWon = freeTotalWon + previousState.freeSpinTotalWon + previousState.mainSpinCreditsWon;

    this.responseObj.freeGame.currentFreeSpin = previousState.freeGame.currentFreeSpin + 1;
    this.responseObj.freeGame.totalFreeSpin = previousState.freeGame.totalFreeSpin;
