...
class ResponseGenerator {
  ...
  createResponse(responseMap, response) {
    ...
    response.reSpinGame.totalRespin = responseMap.get(RESPIN_GAME_TOTAL_SPIN) || 0;
    response.reSpinGame.currentRespin = responseMap.get(RESPIN_GAME_CURRENT_SPIN) || 0;
  ...
  createRespinResponse(responseMap, response) {
    ...
    if (bonusTriggerData) {
      ...
      response.bonus.count = responseMap.get(BONUS_COUNT);
      response.bonus.position = responseMap.get(BONUS_POSITION);
      response.bonus.respinWon = responseMap.get(TOTAL_BONUS_WIN);
      response.reSpinGame.respinGameRetrigger = responseMap.get(RESPIN_GAME_RETRIGGER);
    }
    // //Bonus
