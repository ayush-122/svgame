...
function printResponse() {
  ...
  const respinFeatWinPercentage = (responseObj.totalRespinFeatureWon / responseObj.totalBet) * 100;
  const totalWinPercentage = (responseObj.totalGameWin / responseObj.totalBet) * 100;
  const freeHitPercentage = responseObj.cycles / responseObj.freeHit;
  const reFreeHitPercentage = responseObj.cycles / responseObj.reFreeHit;
  const respinHitPercentage = responseObj.cycles / responseObj.respinHit;
  const reRespinHitPercentage = responseObj.cycles / responseObj.reRespinHit;

  const res = `\nThe Main Output \n****************\n
  BaseWinPct: ${baseWinPercentage.toFixed(2)}% | FreeWinPct: ${freeWinPercentage.toFixed(
    2
  )}% | RespinWinPct: ${respinWinPercentage.toFixed(2)}% |  RespinFeatWinPercentage: ${respinFeatWinPercentage.toFixed(
    2
  )} |TotalWinPct: ${totalWinPercentage.toFixed(2)}% | \n FreeHitPercentage: ${freeHitPercentage.toFixed(
    2
  )} | reFreeHitPercentage: ${reFreeHitPercentage.toFixed(2)} | respinHitPercentage: ${respinHitPercentage.toFixed(
    2
