  ...
  try {
    ...
    for (let i = 1; i <= cycles; i++) {
      ...
      while (nextState == "FREE" && (firstFreeSpin || currentFreeSpin != totalFreeSpin)) {
        ...
        if (freeStateRes.scatter.freeGameRetrigger) reFreeHit++;
      }
      if (nextState == "RESPIN") respinHit++;
      while (nextState == "RESPIN") {
        ...
        let respinCreditsWon = respinStateRes.reSpinGame.baseRespinCreditWon - respinStateRes.featureWin;
        totalRespinWon = totalRespinWon + respinCreditsWon;
        totalRespinFeatureWon = totalRespinFeatureWon + respinStateRes.featureWin;
        totalGameWin = totalGameWin + respinStateRes.reSpinGame.baseRespinCreditWon;
        if (respinStateRes.reSpinGame.respinGameRetrigger) reRespinHit++;
      }
      ...
      if (i % 1000000 == 0) {
        ...
        const respinFeatWinPercentage = (totalRespinFeatureWon / totalBet) * 100;
        const totalWinPercentage = (totalGameWin / totalBet) * 100;
        const freeHitPercentage = i / freeHit;
        const reFreeHitPercentage = i / reFreeHit;
        const respinHitPercentage = i / respinHit;
        const reRespinHitPercentage = i / reRespinHit;

        console.log(
          i,
          `BaseWinPct: ${baseWinPercentage.toFixed(2)}% | FreeWinPct: ${freeWinPercentage.toFixed(
            2
          )}% | RespinWinPct: ${respinWinPercentage.toFixed(
            2
          )}% |  RespinFeatWinPercentage: ${respinFeatWinPercentage.toFixed(
            2
          )} | TotalWinPct: ${totalWinPercentage.toFixed(2)}% | \n FreeHitPercentage: ${freeHitPercentage.toFixed(
            2
          )} | reFreeHitPercentage: ${reFreeHitPercentage.toFixed(
            2
          )} | respinHitPercentage: ${respinHitPercentage.toFixed(
            2
          )} | reRespinHitPercentage: ${reRespinHitPercentage.toFixed(2)}`
        );
      }
    }
