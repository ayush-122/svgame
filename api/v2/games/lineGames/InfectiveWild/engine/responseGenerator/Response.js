...
class ResponseGenerator {
  ...
  createNew() {
    const responseJson = {
      ...
      payline: [],
      featureWin: 0,
      infectiveWildFeature: {
        featureReels: null,
        featurePaylines: [],
        infectiveWild: [0, 0, 0, 0, 0],
        infectionOrder: [],
      },
      scatter: {
        count: 0,
  ...
  createResponse(responseMap, response) {
    ...
    response.infectiveWildFeature.infectiveWild = responseMap.get(INFECTIVE_WILD); //expanding wild
    response.infectiveWildFeature.infectionOrder = responseMap.get(INFECTIVE_POSITION); //expanding wild positions

    response.infectiveWildFeature.featureReels = responseMap.get(FEATURE_REELS);
    response.infectiveWildFeature.featurePaylines = responseMap.get(FEATURE_PAYLINES) || [];
