...
async function postDone(req, res) {
    ...
    if (last_state.length <= 0) 
      return res.status(statusCode.NOT_FOUND).json({ status: false, message: "No last spin found" });
    ...
    return res.status(statusCode.OK).json({ status: true, message: "Cascade Status Updated Successfully" });
  } 
